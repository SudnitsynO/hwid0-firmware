...
{
	unsigned int R = crc ^ (byte << 8);
    for(unsigned char i = 0; i < 8; i++)
		R = R & 0x8000 ? ( R << 1 ) ^ 0x8005 : R << 1;
    return R;
}
...
unsigned long Crc32Block(const std::vector<uint8_t> &buf, unsigned long crc)
{
	return Crc32Block(crc, buf.data(), buf.size());
}

unsigned long Crc32Block(const Buffer &buf, unsigned long crc)
{
	return Crc32Block(crc, buf.get(), buf.size());
}
