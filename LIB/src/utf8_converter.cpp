	...
	for (; i < (int)max_size && utf8[i] != 0; ++i) {
		...
		if ((prefix & 0x80) == 0) {
			...
			if (unicode_char >= 0x410 && unicode_char <= 0x44F) {
				...
				int count = sizeof(g_letters) / sizeof(ConvLetter);
	...
	while(i < max_size)
	{
		...
		else 
		{
			...
			if ((~prefix) & 0x20) {
				...
				if (unicode_char >= 0x410 && unicode_char <= 0x44F) {
					w1251.append(1, (char)(unicode_char - 0x350));
				}
				else if (unicode_char >= 0x80 && unicode_char <= 0xFF) {
					w1251.append(1, (char)(unicode_char));
				}
				else if (unicode_char >= 0x402 && unicode_char <= 0x403) {
					w1251.append(1, (char)(unicode_char - 0x382));
				}
				else {
					int count = sizeof(g_letters) / sizeof(ConvLetter);
					for (int k = 0; k < count; ++k) {
						if (unicode_char == g_letters[k].unicode) {
							w1251.append(1, g_letters[k].win1251);
							break;
						}
					}
				}
